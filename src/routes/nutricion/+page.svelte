<script lang="ts">
  // Prueba del endpoint POST /chat de nutricion_api.
  ...
  // - Diseño final = híbrido mínimo: el resultado son el `platillo` (texto) +
  //   los 4 macros con los nombres OFICIALES del schema macronutrientes_y_kcal
  //   (kilocalorias / proteinas / carbohidratos / grasas).
  // - LECTURA TOLERANTE a la transición del back: si aún manda el schema viejo
  //   (totales.kcal / proteinas_g / ...), igual lo leemos. Así no se rompe la
  //   demo mientras el back migra.
  import { env } from '$env/dynamic/public';

  const API_URL = env.PUBLIC_API_URL ?? 'http://localhost:8000';

  // Macros con los nombres oficiales del schema macronutrientes_y_kcal.
  ...
  type Respuesta = {
    ...
    // schema nuevo (plano, nombres oficiales)
    kilocalorias?: number | null;
    proteinas?: number | null;
    carbohidratos?: number | null;
    grasas?: number | null;
    // schema viejo (anidado) — fallback de transición
  ...
  // Guardado manual (botón "Guardar") por resultado final.
  ...
  // Normaliza los macros leyendo el schema nuevo (plano) o, si no está, el viejo
  // (anidado en `totales`). Devuelve null si no hay macros (p. ej. modo pregunta).
  ...
  // Upsert por conversation_id en el back, así que guardar uno u otro resultado
  // de la misma conversación actualiza la misma fila.
